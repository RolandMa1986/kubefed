...
func GetDeleteOptions(obj *unstructured.Unstructured) []client.DeleteOption {
	...
	if prop, ok := annotations[PropagationPolicyAnnotation]; ok {
		policy := (client.PropagationPolicy)(prop)
		options = append(options, policy)
	}

	if str, ok := annotations[GracePeriodSecondsAnnotation]; ok {
		if seconds, err := strconv.ParseInt(str, 10, 64); err == nil {
			grace := (client.GracePeriodSeconds)(seconds)
			options = append(options, grace)
		}
	}
